...
//connectable or discoverable request, because DTUN
//call calls adapter_mode_changed synchronously
#ifdef BT_ALT_STACK
	if (msg != NULL) {
...
//BRCM change: create pending session is done before dispatching
//connectable or discoverable request above, because DTUN
//call calls adapter_mode_changed synchronously.
					...
					service_auth_cb cb, void *user_data)
{
    ...
    // Instead of writing/reading the GLOBAL_TRUST, we now write and read
    // per service
        char service[32];
