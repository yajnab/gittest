...
#if !defined(LINUX_NATIVE) && !defined(DTUN_STANDALONE)
...
#ifdef DTUN_STANDALONE
#define DTUN_THREADED
typedef void (*tDTUN_PROCESS_HANDLER)(void *);
#else
typedef gboolean (*tDTUN_PROCESS_HANDLER)(GIOChannel *chan, GIOCondition cond, gpointer data);
#endif
...
static const char *id_name[] =
{
	...
	"DTUN_SIG_DM_SSP_KEY_NOTIF",// Luke
    "DTUN_SIG_DM_LINK_UP",
...
static int create_subprocess(int iface, tDTUN_PROCESS_HANDLER handler)
{
#ifdef DTUN_THREADED
    pthread_t thread_id;
    ...
    debug("create_subprocess (threaded) : [%s]\n", iface2name(iface));
    ...
    debug("create_subprocess (glib) : [%s]\n", iface2name(iface));
...
{
    int                 sockfd, n;
#ifdef BTLIF_MAKE_LOCAL_SERVER_NAME
    char local_server_name[UNIX_PATH_MAX];
    BTLIF_MAKE_LOCAL_SERVER_NAME(local_server_name, ip_str, DTUN_PORT);
    if ( (sockfd = socket(AF_LOCAL, SOCK_STREAM, 0)) < 0)
        perror("socket error");
    n = socket_local_client_connect(sockfd, local_server_name, ANDROID_SOCKET_NAMESPACE_ABSTRACT, SOCK_STREAM) >= 0 ? 0 : -1;
    info("connect_srv ret:%d server name:%s\n", n, local_server_name);
#else
...
boolean wait_for_signal(int fd)
{
...

    {
        ...
        if ((index<0) || (iface == DTUN_INTERFACE_INVALID))
        {
...
boolean dtun_client_call_method(tDTUN_DEVICE_METHOD *method)
{
    ...
    if (iface == DTUN_INTERFACE_INVALID)
    {
        error("interface not found for id %d", method->hdr.id);
        return false;
    }
...
#ifndef BTLIF_LOCAL_SERVER_ADDR
...
#ifdef DTUN_THREADED
    create_subprocess(iface, client_signal_handler_threaded);
#else
